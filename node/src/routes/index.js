...
module.exports = function(passport) { // Always send user to jade for navbar to work properly
    ...
    router.get('/signup', function(req, res) {
        res.render('signup', {
            user: req.user,
            error: req.flash('signup-error')
        });
    });
    router.post('/login', function(req, res, next) {
        var error = false;
        if (!req.body.username) {
            req.flash('login-error', 'Sorry. Username can\'t be blank.');
            error = true;
        }
        if (!req.body.password) {
            req.flash('login-error', 'Sorry. Password can\'t be blank.');
            error = true;
        }
        if (error) {
    ...
    router.post('/signup', function(req, res, next) {
        var error = false;
        if (!req.body.username) {
            req.flash('signup-error', 'Sorry. Username can\'t be blank.');
            error = true;
        }
        if (!req.body.username) {
            req.flash('signup-error', 'Sorry. First Name can\'t be blank.');
            error = true;
        }
        if (!req.body.username) {
            req.flash('signup-error', 'Sorry. Last Name can\'t be blank');
            error = true;
        }
        if (!req.body.email) {
            req.flash('signup-error', 'Sorry. Email can\'t be blank');
            error = true;
        }
        if (!req.body.password) {
            req.flash('signup-error', 'Sorry. Password can\'t be blank.');
            error = true;
        }
        if (req.body.username.match(/[^\w]/g)) {
            req.flash('signup-error', 'Sorry. Username must match [a-zA-Z0-9_]');
            error = true;
        }
        if (error) {
            return res.redirect('/signup');
